!function(){
	...
	var c1=[-130, 40], c2=[-50, 140], c3=[-10, 180]; //Column positions of labels.
	...
	function drawPart(data, id, p){
		...
		mainbar.append("rect").attr("class","mainrect")
			...
			.style("stroke","black").style("stroke-opacity",0).style("fill","#DA430A");
			
		mainbar.append("text").attr("class","barlabel")
			.attr("x", c1[p]).attr("y",function(d){ return d.middle+5;})
			.text(function(d,i){ return data.keys[p][i];})
			.attr("text-anchor","start" ).style("fill","#DA430A");
			
		mainbar.append("text").attr("class","barvalue")
			.attr("x", c2[p]).attr("y",function(d){ return d.middle+5;})
			.text(function(d,i){ return d.value ;})
			.attr("text-anchor","end").style("fill","#DA430A");
	...
	function drawHeader(header, id){
		d3.select("#"+id).append("g").attr("class","header").append("text").text(header[2])
			...
			.style("font-weight","bold").style("fill","#DA430A");
		
		[0,1].forEach(function(d){
			var h = d3.select("#"+id).select(".part"+d).append("g").attr("class","header");
			
			h.append("text").text(header[d]).attr("x", (c1[d]-5))
				.attr("y", -5).style("fill","#DA430A");
			
			h.append("text").text("Count").attr("x", (c2[d]-10))
				.attr("y", -5).style("fill","#DA430A");
			
			h.append("line").attr("x1",c1[d]-10).attr("y1", -2)
				.attr("x2",c3[d]+10).attr("y2", -2).style("stroke","#DA430A")
